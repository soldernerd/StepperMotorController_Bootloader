...
def suspend_bootloader():
    tx_data = [0x00, 0x24]
    spi_send_receive(tx_data)
    print('Bootloader supended')
...
def find_file(file_name):
    ...
    if not received_data[:3] == [0x81, 0xC1, 0x25]:
        print('Signature of first data package is incorrect:', list_to_string(received_data[:3]))
        return 0
    else:
...
#delay_ms(500)
test_spi_communication(10,100)
#short_hex = find_file('short.hex')
